// ESPN public JSON adapter. Keyless; polled per (CDN-cached) request, down to
// once a minute while a match is live.
...
// Verified June 23 against live 2026 data (USA-Paraguay, Qatar-Switzerland):
// ESPN's d.team for own goals is the BENEFITING team, not the conceding team.
// Cards use d.team correctly (matches the carded player's team).
export function tallies(comp) {
  const out = {}; // teamId -> { cy, cr, og }
  for (const c of comp.competitors) out[c.team.id] = { cy: 0, cr: 0, og: 0 };
  for (const d of comp.details || []) {
    if (d.ownGoal) {
      // d.team is the team that benefited; the conceding team is in athletesInvolved.
      const ogTid = d.athletesInvolved?.[0]?.team?.id;
      if (ogTid && out[ogTid]) out[ogTid].og += 1;
      continue;
    }
    const tid = d.team?.id;
...
// ESPN's editorial recap for a completed match, if present (47 of 48 group games
// carried one). `short` is the punchy headline, `description` a one-line recap, and
// `url` the recap article (event "recap" link, falling back to "summary"). Returned
// only for completed matches; news.js prefers this over its generated story.
export function matchHeadline(comp, links, completed) {
  if (!completed) return null;
  const hl = (comp.headlines || []).find((h) => h.shortLinkText || h.description);
  if (!hl) return null;
  const link = (links || []).find((l) => (l.rel || []).includes("recap"))
            || (links || []).find((l) => (l.rel || []).includes("summary"));
  const url = link?.href && /^https?:\/\//.test(link.href) ? link.href : null;
  return {
    short: hl.shortLinkText || hl.description || null,
    description: hl.description || null,
    url,
  };
}
...
export async function fetchSeason() {
  ...
  const matches = (sb.events || []).map((e) => {
    ...
    return {
      ...
      headline: matchHeadline(comp, e.links, state === "post"),
    };
  });
