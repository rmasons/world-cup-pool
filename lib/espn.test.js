...
import { tallies, matchHeadline } from "./espn.js";
...
describe("matchHeadline — ESPN recap extraction", () => {
  const comp = { headlines: [{ type: "Recap", shortLinkText: "Mexico win opener", description: "Mexico beat South Africa 2-0." }] };
  const links = [
    { rel: ["summary", "desktop", "event"], href: "https://www.espn.com/soccer/match/_/gameId/9" },
    { rel: ["recap", "desktop", "event"], href: "https://www.espn.com/soccer/report/_/gameId/9" },
  ];

  it("returns short text, description, and the recap url for a completed match", () => {
    expect(matchHeadline(comp, links, true)).toEqual({
      short: "Mexico win opener",
      description: "Mexico beat South Africa 2-0.",
      url: "https://www.espn.com/soccer/report/_/gameId/9",
    });
  });

  it("returns null for an unfinished match or when there are no headlines", () => {
    expect(matchHeadline(comp, links, false)).toBe(null);
    expect(matchHeadline({ headlines: [] }, links, true)).toBe(null);
  });

  it("prefers the recap link but falls back to summary", () => {
    expect(matchHeadline(comp, [{ rel: ["summary"], href: "https://www.espn.com/x" }], true).url)
      .toBe("https://www.espn.com/x");
  });

  it("drops a non-http url (no javascript: or other schemes)", () => {
    expect(matchHeadline(comp, [{ rel: ["recap"], href: "javascript:alert(1)" }], true).url).toBe(null);
  });
});
