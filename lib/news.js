...
function storyFor(m, poolTeams) {
  // Prefer ESPN's real recap headline when the feed carries one; fall back to the
  // generated box-score story. `url` (when present) drives a "via ESPN" link.
  if (m.headline?.short) {
    return {
      h: m.headline.short,
      s: m.headline.description || `${scoreline(m)}.`,
      url: m.headline.url || null,
    };
  }
...
export function buildContent(matches, rosterTeams, statsMap) {
  ...
  const upGames = [
    ...todays.map((m) => ({ iso: m.dateUTC, d: shortDate(m.dateUTC), k: kickoff(m.dateUTC), a: lookup.toRoster(m.home.name) || displayName(m.home.name), b: lookup.toRoster(m.away.name) || displayName(m.away.name), v: m.venueCity, td: 1, ...(m.live ? { live: 1, min: m.clock, sa: m.home.score ?? 0, sb: m.away.score ?? 0 } : {}), _id: m.id, _stage: m.stage })),
    ...future.slice(0, 3).map((m) => ({ iso: m.dateUTC, d: shortDate(m.dateUTC), k: kickoff(m.dateUTC), a: lookup.toRoster(m.home.name) || displayName(m.home.name), b: lookup.toRoster(m.away.name) || displayName(m.away.name), v: m.venueCity, td: 0, _id: m.id, _stage: m.stage })),
  ].slice(0, 9);
  ...
  for (const t of rosterTeams) {
    ...
    teamFx[t] = matches
      ...
      .slice(0, 2)
      .map((m) => {
        const isHome = lookup.toRoster(m.home.name) === t;
        const opp = isHome ? m.away : m.home;
        return {
          iso: m.dateUTC,
          d: shortDate(m.dateUTC),
          k: kickoff(m.dateUTC),
          o: lookup.toRoster(opp.name) || displayName(opp.name),
          v: m.venueCity,
          // su/so: this team's score and the opponent's, so the row can read
          // "vs Opponent 1–0" from the roster team's perspective.
          ...(m.live ? { live: 1, min: m.clock, su: (isHome ? m.home : m.away).score ?? 0, so: opp.score ?? 0 } : {}),
        };
      });
  }
