...
export function buildContent(matches, rosterTeams, statsMap) {
  ...
  const upGames = [
    ...todays.map((m) => ({ d: shortDate(m.dateUTC), k: kickoff(m.dateUTC), a: lookup.toRoster(m.home.name) || displayName(m.home.name), b: lookup.toRoster(m.away.name) || displayName(m.away.name), v: m.venueCity, td: 1, _id: m.id, _stage: m.stage })),
    ...future.slice(0, 3).map((m) => ({ d: shortDate(m.dateUTC), k: kickoff(m.dateUTC), a: lookup.toRoster(m.home.name) || displayName(m.home.name), b: lookup.toRoster(m.away.name) || displayName(m.away.name), v: m.venueCity, td: 0 })),
  ].slice(0, 9);
