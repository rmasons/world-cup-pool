// Pre-game win probabilities for every game on the schedule, from The-Odds-API
// h2h markets (free tier; one call covers all listed events, ~1 credit per
// update). Entirely optional: if the key is missing, the call fails, or an
// event isn't priced yet, games simply render without odds.
import { deburr } from "./teams.js";

const SPORT = "soccer_fifa_world_cup";

// Warm-instance memo: with live scores the pool rebuilds as often as once a
// minute, but pregame odds don't move that fast — reuse the last fetch for 15
// minutes to protect the 500-credits/month free tier.
let memo = { at: 0, events: null };
const MEMO_TTL = 15 * 60 * 1000;

export async function attachOdds(upGames) {
  const key = process.env.ODDS_API_KEY;
  if (!key || !upGames.length) return upGames.map(strip);

  let events = Date.now() - memo.at < MEMO_TTL ? memo.events : null;
  if (!events) {
    try {
      ...
      // odds are decorative — never fail the update over them. A stale memo
      // beats no odds at all.
      if (!memo.events) return upGames.map(strip);
      events = memo.events;
    }
  }
